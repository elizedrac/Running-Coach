...
- pacing_calculator: {{"goal_time": "HH:MM:SS or MM:SS", "distance": float miles (not km), "race_type": "optional"}} — ALL args optional: only include values the user actually stated; anything missing is auto-filled from their saved race. NEVER invent or convert values — "5k"/"10k" name a DISTANCE, never a goal_time; a race name alone means goal_time is omitted, the system will handle it. If distance is missing but a race_type is mentioned, derive it using one of: {", ".join(RACE_DISTANCES_KNOWLEDGE.keys())}; otherwise leave blank.
