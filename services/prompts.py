...
TOOL_METADATA = {
    ...
    "pacing_calculator": "Calculate target training/race paces from a goal time and/or distance. Use for any pace question, even with only one of the two — missing args get prompted for. Do NOT use if the conversation is mid-discussion about updating the plan.",
...
- pacing_calculator: {{"goal_time": "HH:MM:SS or MM:SS", "distance": float miles (not km), "race_type": "optional"}} — if distance is missing, derive it from race_type using one of: {", ".join(RACE_DISTANCES_KNOWLEDGE.keys())}; otherwise leave blank.
