...
TOOL_METADATA = {
    ...
    "race_prep_info": "Retrieve race day preparation knowledge — nutrition plan, race morning routine, warm-up, pacing strategy, and gear advice. Use only when the user explicitly asks about race day prep, what to eat before/during a race, race morning routine, warm-up protocol, or how to execute the race itself.",
...
- If the user completed the planned workout (miles >= target), check if the workout_type classification is correct: re-evaluate using actual average_pace vs pacing zones (if available). If the pace indicates a clearly harder effort than the planned type (e.g. plan is EASY but actual pace is in tempo zone), upgrade the workout_type and update notes. Do NOT use avg_hr to reclassify — HR is unreliable on treadmills. Otherwise mark it as done — no change needed.
...
- If they ran on a REST day (running only, no strength): (1) Classify effort using pace vs pacing zones (if pacing data is available, compare actual average_pace to tempo/aerobic/easy pace ranges; without pacing data, default to AEROBIC). Do NOT use avg HR to classify effort — HR is unreliable on treadmills due to heat. Set target_miles and target_pace to actual values (skip pace update if treadmill). Add a note "Unplanned [TYPE] logged." (2) Rebalance remaining future days — RULES: (a) The day immediately after an unplanned hard effort (TEMPO, INTERVAL, LONG) MUST be EASY or REST — never upgrade it. (b) If a day of the same hard type exists later in the week, downgrade it to EASY or REST — weekly quota already fulfilled. (c) NEVER upgrade any future day to a harder type as part of rebalancing — only downgrade or keep. (d) No two hard days on consecutive dates. Goal: a recoverable week, not redistributing load upward.
