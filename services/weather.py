...
def get_weather(user_id: str, location: str = DEFAULT_LOCATION, date: str = None) -> str:
    if date and date != dt_date.today().isoformat() and date.strip().lower() != 'today':
        return "Historical and forecasted weather data is not supported in this version. Can only fetch current weather."
    ...
    try:
        ...
        next_12 = [hour for hour in hours if datetime.strptime(hour["time"], "%Y-%m-%d %H:%M").hour >= now][:12] # Get first 12 hours of forecast
