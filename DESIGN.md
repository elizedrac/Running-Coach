...
| status | meaning |
|---|---|
| `success` | every change written |
| `partial` | some written, some in `failed` (each with the reason it was rejected) |
| `fail` | nothing written |
| `no_changes` | the plan already matched the request. `update_plan_day` returns `success` for an empty list, so this used to reach the coach as success-with-no-changes and be reported as a failure |
| `out_of_window` | every requested date fell outside the writable range; the dates are in `out_of_range` |

- **Per-change validation.** Changes are validated one at a time, not as a batch. Validating `UpdatePlanOutput` whole meant one malformed day (a notes-only edit with no `workout_type`, a rep missing `interval_num`) raised and discarded every other change in the payload, defeating the per-day `applied`/`failed` reporting `update_plan_day` exists to provide. Rejects go to `failed` carrying the field path that failed.
- **`extract_json()` (`services/llm.py`) parses every model reply**, replacing `response[first_brace:last_brace]`. A model that echoed the `{"changes": []}` example from its own SCOPE line, wrote a sentence, then gave the real payload produced one unparseable string and the edit was thrown away. The helper scans balanced top-level objects, skips braces inside strings, and keeps the largest that parses. Used by `planner`, `sql_selector`, `write_selector`, `race_info`, `course_details`, `end` and `plan`.
- **An INTERVAL day must carry its reps.** A change setting `INTERVAL` without `intervals` matched neither branch in `update_plan_day`: the day saved, the rep table was left untouched, and it still counted as applied — producing interval days with no breakdown. Skipping is still correct when the day already has reps; when it has none the day is reported in `interval_failures`.
- **Orphaned locks clear on boot.** Locks release in a `finally`, which cannot run if the process dies, so a restart or OOM locked the user out for the full 40 min TTL with nothing running — and Redis's anonymous volume at `/data` brought the stale keys back across a container recreate. A `lifespan` hook in `main.py` clears `plan_job_lock`, `garmin_sync_lock`, `chatlock` and `chatcancel` at startup, when no job can be in flight. `chatstream` is deliberately left: it is replayable content a reload reattaches to, not a lock. Safe for one app container; two would need a heartbeat that re-stamps a short TTL instead.
- **Orphaned status blobs resolve on boot too** (`_resolve_orphaned_job_status` in `main.py`). A job's status outlives its lock — the lock expires in 40 min, the blob in 24 h — so clearing only the lock left `{"status": "running"}` readable for a day and a reattaching page polled a job that had died with the old process, spinning until the tab closed. The two blobs are resolved differently because their pollers read a missing key differently: `pollSyncStatus` treats `idle` as finished and stops cleanly, so `garmin_sync_status` is deleted; `pollPlanJob` hands whatever it reads to `planCreateDone`/`planSyncDone`, which word their toast off the status, so `plan_job_status` is rewritten to an `error` (preserving `kind`, which selects the handler) rather than dropping to a silent `idle`. Only `running` is stale by definition — a terminal blob is a finished job's result and is left alone.
...
- **Progress is logged, not published** (`_drain_plan_stream` in `services/plan.py`). `get_final_message()` blocks until the whole plan lands, which on a 28-week race is minutes of silence — a job that died in there was indistinguishable in the logs from one that never started. The stream is drained event by event instead, emitting `create_plan_phase` with `phase` (`thinking` → `writing` → `written`, or `revising` once the challenger has rejected a plan), `iteration`, `days_done` and `days_total`. Days are counted by occurrences of the `plan_date` key in the joined `input_json_delta` buffer, not per chunk — a delta can split the key across two events. `save_training_plan` sets `eager_input_streaming: True`: without it the API buffers the whole days array and delivers it in one burst, so a measured 193-day run generated for 3.5 min with nothing on the wire and then landed in a second, leaving the count at 0 throughout. It changes delivery only — the accumulated tool input parses identically, at the cost of one prompt-cache rewrite when the tool definition changed. Throttled to `PHASE_LOG_INTERVAL` (10s) while writing, with a closing `written` line so a fast plan does not only ever report `days_done=0`. Deliberately log-only: the status blob and the UI progress bar are unchanged, since the value here is diagnosing a failed create after the fact
- **An empty plan explains itself** (`emptyPlanPlaceholder` in `static/index.html`). An empty current week is ambiguous — never created one, or the last create died — so the placeholder reads the job status blob and says "your last plan attempt didn't finish" when the last `create` ended in `error`/`fail`. Read on every render rather than fired once as a toast, so it needs no dedupe, no `localStorage` and no ack route; the raw `str(e)` in the blob is deliberately not shown to the user
...
- **Scope**: reads a fixed ±7/+8 day window; the *writable* range depends on mode. Chat: today−7 → today+8. Sync: this week's Monday → **today**, with one day of grace so a Monday sync can still reconcile Sunday's run. Weekly refresh: this week's Monday → the coming Sunday, no grace. Sync's ceiling is today because reconciliation reports what happened and must not re-plan what has not: it previously ran to the coming Sunday, so a Monday sync could rewrite six days ahead, and did. If a completed day's load warrants easing a future day, sync says so in the completed day's notes and leaves the change to chat. Changes outside the writable range are dropped into `out_of_range`; the coach names those dates and directs the user to click the day or regenerate the plan. **Both** bounds are passed into `build_update_plan_system(earliest=..., latest=...)` so the prompt and the filter always agree — the ceiling used to go unstated, so the model proposed days it was never allowed to write and they were silently dropped
